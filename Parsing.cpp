	...
	if (f_nf->is_open()) {
		...
		while (!f_nf->eof()) {
			...
			for (int j = 0; j < seq.size() - k + 1; j++) {
				...
				if (!am->kmerFreq->count(kmer)) {am->kmerFreq->insert(make_pair(kmer, 1));}	else {am->kmerFreq->insert_or_assign(kmer, am->kmerFreq->at(kmer) + 1);}
...
void Parsing::Qread(const string& filename, const bool& reportMultipleHits)
{
	...
	std::cout << "average read length is " << (int)avg << " bases" << endl; 
	...
	for (int m = 0; m < numT; m++) {
		...
		Framegen* fg = new Framegen(*nuciomap);
		string frames[6] = { fg->gf(fk), fg->gf(fk.substr(1)), fg->gf(fk.substr(2)), fg->gf(rk),fg->gf(rk.substr(1)), fg->gf(rk.substr(2)) };
	...
	if (f_in->is_open()) {
		...
		while (getline(*f_in, tp)) {
			...
			if ((seq.length() / 3) > k) {
				...
				delete sq, fg, kmerHitsMandatoryBest, kmerHitsBest, geneHitsMandatoryWeightedBest, geneHitsWeightedBest, geneHitsMandatoryUnweightedBest, geneHitsUnweightedBest, bestMandatory;
			}
			if (tc % 100000 == 0) {
